...
void DestroyEntityPool(EntityPool* pool) {
    ...
    // Free active flags array
    if (pool->active) {
        free(pool->active);
        pool->active = NULL;
    }
    
    // Note: Don't free registry or world as they're managed externally
    pool->registry = NULL;
    pool->world = NULL;
...
Entity* CreateEntity(EntityPool* pool, EntityType type, Vector2 position) {
    if (!pool || !pool->entities || !pool->active) {
        if (pool) pool->status = POOL_INVALID_ENTITY;
        return NULL;
    }
    
    // Find first free slot
    size_t slot = pool->firstFree;
    while (slot < pool->capacity && pool->active[slot]) {
        slot++;
    }
    ...
    pool->count++;
    pool->firstFree = slot + 1;
    
    // Initialize entity
    Entity* entity = &pool->entities[slot];
...
void RemoveEntity(EntityPool* pool, Entity* entity) {
    if (!pool || !entity || !pool->entities || !pool->active) {
        if (pool) pool->status = POOL_INVALID_ENTITY;
        return;
    }
    ...
    if (index >= pool->capacity) {
        pool->status = POOL_INVALID_ENTITY;
        return;
    }
    
    // Only remove if the entity is active
    ...
    // Update first free slot if this is earlier in the array
...
void UpdateEntityPool(EntityPool* pool, World* world, float deltaTime) {
    if (!pool || !world || !pool->entities || !pool->active) {
        if (pool) pool->status = POOL_INVALID_ENTITY;
        return;
    }
    ...
    for (size_t i = 0; i < pool->capacity; i++) {
        ...
        if (!entity || !entity->Update) continue;  // Added NULL check
        
        // Validate entity state before update
        if (!entity->active) continue;
        
        // Protect against invalid memory access
        if (entity->type >= ENTITY_TYPE_COUNT) {
            pool->active[i] = false;
            pool->count--;
            continue;
        }
        ...
        // Validate position after update to prevent out-of-bounds
...
void DrawEntityPool(EntityPool* pool) {
    ...
    for (size_t i = 0; i < pool->capacity; i++) {
        ...
        Entity* entity = &pool->entities[i];
        if (!entity || !entity->visible || !entity->Draw) continue;  // Added NULL check
        
        // Validate entity state before drawing
        if (!entity->active) continue;
...
Entity* GetEntityAt(EntityPool* pool, Vector2 position, float radius) {
    ...
    for (size_t i = 0; i < pool->capacity; i++) {
        ...
        Entity* entity = &pool->entities[i];
        if (!entity || !entity->active) continue;  // Added NULL check and active check
        
        float distance = Vector2Distance(entity->position, position);
...
Entity* GetEntityByType(EntityPool* pool, EntityType type) {
    if (!pool || !pool->entities || !pool->active) return NULL;
...
static void InitializePool(EntityPool* pool, size_t capacity) {
    ...
    pool->entities = (Entity*)AlignedAlloc(capacity * sizeof(Entity), POOL_MEMORY_ALIGNMENT);
    if (!pool->entities) {
        pool->status = POOL_OUT_OF_MEMORY;
        return;
    }
    ...
    pool->firstFree = 0;
    pool->status = POOL_OK;
    pool->registry = NULL;  // Will be initialized by the world
    pool->world = NULL;     // Will be set by the world
    
    // Zero-initialize all entities and active flags
    memset(pool->entities, 0, capacity * sizeof(Entity));
    memset(pool->active, 0, capacity * sizeof(bool));
}
